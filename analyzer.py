...
class attom:
    def __init__(self):
        ...
        self.analysis_file_name = str(datetime.datetime.today()).split('.')[0].replace('-','').replace(' ','-').replace(':','')+'.anl'
        f_o = open(self.analysis_file_name,'w')
        f_o.close()
    ...
    def binary2txt(self):
        f_o,jobid = open(self.analysis_file_name,'a'),1
        data_list = [i for i in glob.glob('*') if not '.py' in i and not '.txt' in i and not '.csv' in i and not '.xls' in i and not '.anl' in i]
        data_list = sorted(data_list)
        if 'data' in data_list:del data_list[data_list.index('data')]
        if 'figure' in data_list:del data_list[data_list.index('figure')]
        ...
        for fname in data_list:
            ...
            f_o.write('JOB_id = data '+str(jobid)+' '+fname+'* > '+fname+'.txt > data'+'\n')
            jobid += 1
            fo.close()
    ...
    def txt2xls(self):
        ...
        for sheetname in glob.glob(filepath):
            ...
            f_o.write('JOB_id = integrate '+str(jobid)+' '+sheetname.split('/')[-1]+'\n')
            jobid += 1
    ...
    def txt2png(self):
        ...
        for fname in data_list:
            ...
            for j in range(len(array.T)-2):
                ...
                f_o.write('JOB_id = plot '+str(jobid)+' '+name+'.png > figure'+'\n')
                jobid += 1
    ...
    def analysis(self):
        ...
        if judge == 'y':
            ...
            for i in f:
                if i.rstrip().split(',')[1] == 'blank':
                    ...
                    print(pycolor.RED+'Queue'+pycolor.END+' =',pycolor.BLUE+i.rstrip().split(',')[0]+pycolor.END,'/ Sample ID =',pycolor.BLUE+i.rstrip().split(',')[1]+pycolor.END,'/ Sample name =',pycolor.BLUE+i.rstrip().split(',')[2]+pycolor.END)
                    lane = 0
        ...
        print(pycolor.REVERCE+pycolor.BLUE+self.analysis_file_name+pycolor.REVERCE+pycolor.END)
